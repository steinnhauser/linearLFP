...
if __name__ == '__main__':
    ...
    for name, size in zip(population_names, population_sizes):
        ...
        for cell in network.populations[name].cells:
            idx = cell.get_rand_idx_area_norm(section='allsec', nidx=nidx)
            for n, i in enumerate(idx):
                ...
                saved_outside_input[cell.gid, n] = [i, times]
    ...
    if RANK == 0 and save_stim_synapses:
        ...
        ###### write as a h5 version
        nm = "outsideSpikes.h5"
        hdf_file = h5py.File(OUTPUTPATH+nm,'w')
        for i in range(numCells):
            grp=hdf_file.create_group(f'{i}')
            vals = np.zeros(nidx, dtype=object) # contain the various values
            for j in range(nidx):
                lst = right_list[i][j][1].tolist()
                idx = int(right_list[i][j][0])
                lst.insert(0,idx)
                grp.create_dataset(f"{j}",data=lst)
